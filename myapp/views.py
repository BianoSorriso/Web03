...
from .models import PixParceiro
...
def contato(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('contato')  # redireciona pra rota do contato (GET)
        else:
...
def donation_info(request):
    doacao = DonationInfo.objects.first()
    return render(request, "app/doacao.html", {"idBody": "doacao", "doacao": doacao})
...
def doacao(request):
    pix_parceiros = PixParceiro.objects.all()
    doacao = DonationInfo.objects.first()
